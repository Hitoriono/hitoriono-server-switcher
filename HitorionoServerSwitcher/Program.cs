    ...
    static class Program
    {
        ...
        static void Main()
        {
            AppDomain.CurrentDomain.UnhandledException += (sender, args) =>
            {
                ...
                MessageBox.Show("Unhandled exception: " + args.ExceptionObject + "\n\n" + (ReportExceptions ? "The error has been reported to Hitoriono." : "Please report the error to Hitoriono."), "Oh no!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Environment.Exit(1);
            };
            ...
            var logfile = new NLog.Targets.FileTarget("logfile") {
                FileName = "HitorionoServerSwitcher.log", Layout = "${longdate}:${level}:${message} ${exception:format=ToString}",
            ...
            using (Mutex mutex = new Mutex(true, guid, out firstProcess))
            {
                ...
                {
                    MessageBox.Show("Hitoriono Server Switcher is already running", "Opsie", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
