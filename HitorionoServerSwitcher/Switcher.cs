...
namespace HitorionoServerSwitcher
{
    ...
    class Switcher
    {
        ...
        {
            Logger.Info("Disconnecting from Hitoriono");
            ...
            await hostsFile.Parse();
            if (IsConnectedToRipple())
                throw new HumanReadableException("Can't delete entries. Disable antivirus.", "The hosts file looks writable, but Hitoriono Server Switcher wasn't able to delete the entries from it and switch back to osu!. There is most likely a software blocking access to the hosts file. Please disable your antivirus, third party firewall or any similar software that may block edits on the hosts file.");
            Logger.Info("Successfully disconected from Hitoriono");
        }
        
        public async Task ConnectToRipple()
        {
            Logger.Info("Connecting to Hitoriono");
            ...
            if (!IsConnectedToRipple())
                throw new HumanReadableException("Can't write entries. Disable antivirus.", "The hosts file looks writable, but Hitoriono Server Switcher wasn't able to write the required entries to it and switch to Hitoriono. There is most likely a software blocking access to the hosts file. Please disable your antivirus, third party firewall or any similar software that may block edits on the hosts file.");
            Logger.Info("Successfully connected to Hitoriono");
        }
